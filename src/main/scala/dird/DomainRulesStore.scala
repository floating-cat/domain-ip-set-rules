...
object DomainRulesStore:
  ...
  def load(useLocalDataFile: Boolean): DomainRules =
    val geoSites =
      Using.resource(
        if useLocalDataFile then FileInputStream(geoSitesLocalDataPath)
        else URI.create(geoSitesDataUrl).toURL.openStream
  ...
  private def toTypeInt(`type`: Domain.Type): String = `type` match
    ...
    case Domain.Type.Unrecognized(domainTypeNumber) =>
      throw IllegalStateException(s"unrecognized domain type number '$domainTypeNumber")
